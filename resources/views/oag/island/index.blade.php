...
<div class="container mt-5">
    ...
    <div class="text-center mb-3">
        <a href="{{ route('crime.island.create') }}" class="btn btn-lg px-5" style="background: radial-gradient(circle at top left, #ff6f61, #de1c1c); color: #fff; border-radius: 30px; font-weight: bold; box-shadow: 0 6px 12px rgba(0, 0, 0, 0.3); transition: background 0.4s, transform 0.3s;">
            Create New Island
        </a>
    </div>
    ...
    <div class="table-responsive shadow-lg rounded-3 overflow-hidden" style="background: linear-gradient(145deg, #f1f1f1, #ffffff); border: 2px solid #007bff; border-radius: 20px;">
        <table class="table table-striped table-hover" id="island-table" style="border-collapse: separate; border-spacing: 0; border-radius: 20px; overflow: hidden;">
...
$(document).ready(function() {
    if ($.fn.DataTable.isDataTable('#island-table')) {
        $('#island-table').DataTable().clear().destroy();
    }

    $('#island-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route('crime.island.datatables') }}',
        columns: [
            { data: 'id', name: 'id' },
                    ...
                    return `
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton${row.id}">
                                <li>
                                    <a class="dropdown-item" href="${"{{ route('crime.island.edit', ':id') }}".replace(':id', row.id)}">Edit</a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="${"{{ route('crime.island.show', ':id') }}".replace(':id', row.id)}">Show</a>
                                </li>
                                <li>
                                    ...
                                    <form id="delete-form-${row.id}" action="${"{{ route('crime.island.destroy', ':id') }}".replace(':id', row.id)}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </li>
