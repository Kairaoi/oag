...
<div class="container mt-5">
    ...
    <!-- Create New Offence Button -->
    <div class="text-center mb-3">
        <a href="{{ route('crime.offence.create') }}" class="btn btn-lg px-5" style="background: radial-gradient(circle at top left, #ff6f61, #de1c1c); color: #fff; border-radius: 30px; font-weight: bold; box-shadow: 0 6px 12px rgba(0, 0, 0, 0.3); transition: background 0.4s, transform 0.3s; transform: perspective(1px) translateZ(0);">
            Create New Offence
        </a>
    </div>
    ...
    <div class="table-responsive shadow-lg rounded-3 overflow-hidden" style="background: linear-gradient(145deg, #f1f1f1, #ffffff); border: 2px solid #007bff; border-radius: 20px;">
        <table class="table table-striped table-hover" id="offence-table" style="border-collapse: separate; border-spacing: 0; border-radius: 20px; overflow: hidden;">
...
$(document).ready(function() {
    if ($.fn.DataTable.isDataTable('#offence-table')) {
        $('#offence-table').DataTable().clear().destroy();
    }

    $('#offence-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route('crime.offence.datatables') }}',
        columns: [
                    ...
                    return `
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton${row.id}">
                                <li>
                                    <a class="dropdown-item" href="${"{{ route('crime.offence.edit', ':id') }}".replace(':id', row.id)}">Edit</a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="${"{{ route('crime.offence.show', ':id') }}".replace(':id', row.id)}">Show</a>
                                </li>
                                <li>
                                    ...
                                    <form id="delete-form-${row.id}" action="${"{{ route('crime.offence.destroy', ':id') }}".replace(':id', row.id)}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </li>
        ...
        language: {
            search: "_INPUT_",
            searchPlaceholder: "Search offences..."
        },
        responsive: true
    });
});
