...
<div class="container mt-5">
    ...
    <form action="{{ route('crime.accused.update', $accused->id) }}" method="POST" class="p-4 shadow-lg rounded" style="background: linear-gradient(90deg, #ff416c, #ff4b2b); border-radius: 20px;">
        ...
        <div class="form-group">
                        ...
                        @foreach($offences as $id => $name)
                            <div class="form-check">
                                <input 
                                    ...
                                    {{ in_array($id, old('offences', $accusedOffences ?? [])) ? 'checked' : '' }}
                                >
            ...
            <div class="form-check">
                <input type="checkbox" id="add_custom_offence" class="form-check-input" {{ $accused->custom_offence ? 'checked' : '' }}>
            ...
            <input type="text" name="custom_offence" id="custom_offence_input" class="form-control mt-2" 
                   placeholder="Enter custom offence" 
                   value="{{ old('custom_offence', $accused->custom_offence) }}" 
                   style="display: {{ $accused->custom_offence ? 'block' : 'none' }};">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Show custom offence input if there's a value or checkbox is checked
        if (document.getElementById('add_custom_offence').checked || document.getElementById('custom_offence_input').value.trim() !== '') {
            document.getElementById('custom_offence_input').style.display = 'block';
            document.getElementById('add_custom_offence').checked = true;
        }
    });
</script>
@endpush