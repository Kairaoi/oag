...
<div class="container mt-5">
    ...
    <div class="text-center mb-3">
        <a href="{{ route('crime.court-hearings.create') }}" class="btn btn-lg px-5" style="background: radial-gradient(circle at top left, #ff6f61, #de1c1c); color: #fff; border-radius: 30px; font-weight: bold; box-shadow: 0 6px 12px rgba(0, 0, 0, 0.3); transition: background 0.4s, transform 0.3s;">
...
$(document).ready(function() {
    ...
    $('#court-hearing-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route('crime.court-hearings.datatables') }}',
        columns: [
            { data: 'id', name: 'id' },
                    ...
                    return `
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton${row.id}">
                                <li>
                                    <a class="dropdown-item" href="${"{{ route('crime.court-hearings.edit', ':id') }}".replace(':id', row.id)}">Edit</a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="${"{{ route('crime.court-hearings.show', ':id') }}".replace(':id', row.id)}">Show</a>
                                </li>
                                <li>
                                    ...
                                    <form id="delete-form-${row.id}" action="${"{{ route('crime.court-hearings.destroy', ':id') }}".replace(':id', row.id)}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </li>
